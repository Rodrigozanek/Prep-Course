...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  let fraseEspejo = str.split(' ');
  for(var i = 0; i < fraseEspejo.length; i++){
    //let PalabraArray = fraseEspejo[i].split(''); --> ['T', 'h', 'e']
    //let reversePalabra = PalabraArray.reverse(); --> ['e', 'h', 'T']
    //fraseEspejo [i] = reversePalabra.join(''); --'ehT'
    fraseEspejo[i] = fraseEspejo[i].split('').reverse().join('');
  }
  return fraseEspejo.join(' '); //--> 'ehT yrneH egnellahC si !esolc'
} 
